...
// For testing via GET
if (isset($_GET['cardNoField'])) $_POST['cardNoField'] = $_GET['cardNoField'];
if (isset($_GET['pin'])) $_POST['pin'] = $_GET['pin'];
...
if (isset($data['error']) && ($data['error'] == true || $data['error'] == 1)) {
	...
	$sql = "SELECT * FROM membership WHERE userid='".$_POST['cardNoField']."' ORDER BY library_record_index;";

	/*
	if (!$result = $mysqli->query($sql)) {
	    // Oh no! The query failed. 
	    echo "The website is experiencing problems.";
	...
	if ($result = $mysqli->query($sql)) {
		while($row = $result->fetch_assoc()) {
			...
			if ($invalid) {
				...
				//echo "<h1>DELETING: $delQuery</h1>";
				if ($delResult = mysqli_query($con, $delQuery)) {
					//Successful
				} else {
